...
def find_occ_mask(disp_left, disp_right):
    ...
    # # left occlusion
    # remove nan and inf
    inv_mask_l = np.isnan(disp_left)
    inv_mask_l = np.logical_or(inv_mask_l, ~np.isfinite(disp_left))
    disp_left[inv_mask_l] = 0.0
    ...
    right_shifted = right_shifted.astype(int)
    ...
    output_l = (np.ones_like(occ_mask_l) * 255).astype(int)
    output_l[occ_mask_l] = 128
    output_l[inv_mask_l] = 0

    # import matplotlib.pyplot as plt
    # plt.imshow(output_l, cmap='gray')
    # plt.show()

    # # right occlusion
    # remove nan and inf
    inv_mask_r = np.isnan(disp_right)
    inv_mask_r = np.logical_or(inv_mask_r, ~np.isfinite(disp_right))
    disp_right[inv_mask_r] = 0.0
    # find corresponding pixels in target image
    coord = np.linspace(0, w - 1, w)[None,]  # 1xW
    ...
    left_shifted = left_shifted.astype(int)
    ...
    output_r = (np.ones_like(occ_mask_r) * 255).astype(int)
    output_r[occ_mask_r] = 128
    output_r[inv_mask_r] = 0

    # import matplotlib.pyplot as plt
    # plt.imshow(output_r, cmap='gray')
    # plt.show()
