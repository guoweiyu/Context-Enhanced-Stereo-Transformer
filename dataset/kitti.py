...
class KITTIBaseDataset(data.Dataset):
    ...
    def _read_data(self):
        ...
        self.hazardous_data = [img.replace(self.left_fold, self.hazardous_fold) for img in self.left_data]
        self._split_data()
    ...
    def __getitem__(self, idx):
        ...
        if self.hazardous == True:
            hazardous_fname=self.hazardous_data[idx]
            hazardous_occ=np.array(Image.open(hazardous_fname)).astype(np.uint8)
            input_data['hazardous'] = hazardous_occ
...
class KITTI2015Dataset(KITTIBaseDataset):
    def __init__(self, datadir, split='train'):
        ...
        self.hazardous_fold="hazardous/trans_mask/"
        self._read_data()
